...
export default function AdminBookings() {
  ...
  const filtered = bookings.filter(b => {
    ...
    if (search) {
      ...
      return `${b.first_name} ${b.last_name} ${b.email} ${b.title}`.toLowerCase().includes(q);
    }
    return true;
  });
  ...
  const grouped = filtered.reduce((acc, b) => {
    ...
    if (!acc[key]) acc[key] = { title: b.title, day: b.day_of_week, time: b.start_time, instructor: b.instructor, bookings: [] };
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <input placeholder="Search student or class…" value={search} onChange={e => setSearch(e.target.value)}
          className="border border-gray-300 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 w-64" />
      ...
      <div className="space-y-6">
        {Object.values(grouped).sort((a, b) => a.day - b.day || a.time.localeCompare(b.time)).map(group => (
          <div key={group.title} className="bg-white rounded-xl shadow overflow-hidden">
            <div className="bg-brand-600 text-white px-5 py-3 flex items-center gap-3">
              <h3 className="font-semibold">{group.title}</h3>
              <span className="text-brand-200 text-sm">{DAYS[group.day]} · {group.time} · {group.instructor}</span>
              <span className="ml-auto text-xs bg-white/20 px-2 py-0.5 rounded-full">{group.bookings.filter(b => b.status === 'confirmed').length} confirmed</span>
            </div>
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-gray-400 text-xs uppercase">
                <tr>
                  {['Student', 'Email', 'Status', 'Booked At', 'Action'].map(h => (
